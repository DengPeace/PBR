//
//  assimpInstanceLoader.cpp
//  TryGL
//
//  Created by 邓和平 on 2026/2/24.
//
...
InstancedMesh* AssimpInstanceLoader::processMesh(aiMesh* aimesh, const aiScene* scene, const std::string& rootPath, int instanceCount) {
    ...
    if (aimesh->mMaterialIndex >= 0) {
        ...
        if (texture == nullptr) {
            texture = Texture::createTexture("assets/texture/defaultTexture.jpg", 0);
        }
        material->mDiffuse = texture;
    }
    else {
        material->mDiffuse = Texture::createTexture("assets/texture/defaultTexture.jpg", 0);
    }
