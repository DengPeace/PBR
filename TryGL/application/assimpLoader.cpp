...
//    const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenNormals);
    const aiScene* scene = importer.ReadFile(path,
        ...
        aiProcess_GenSmoothNormals |  // 关键：生成平滑法线
...
Mesh* AssimpLoader::processMesh(aiMesh* aimesh, const aiScene* scene, const std::string& rootPath) {
    ...
    if (aimesh -> mMaterialIndex >= 0) {
        ...
        if (texture == nullptr) {
            material -> mDiffuse = new Texture("assets/texture/defaultTexture.jpg", 0);
        }
//        texture -> setUnit(0);
...
//            material -> mSpecularMask = new Texture("assets/texture/defaultTexture.jpg", 0);
//        }
...
Texture* AssimpLoader::processTexture(
    ...
    if (aitexture) {
        ...
        uint32_t widthIn = aitexture -> mWidth; // 通常情况下(png, jpg), 代表了整张图片的大小
